...
<template>
  <div class="memo-item" style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
    <!-- 输入框 -->
    <input
      :value="item.content"
      @input="onInput"
    ...
    <!-- 显示时间或花费 -->
    <div>
      <span v-if="!item.completed">创建：{{ formatDateShort(item.createdAt) }}</span>
      <span v-else>花费：{{ elapsedTime() }}</span>
    </div>
